...
void APortal::Setup()
{
	...
	if (!pawn)
	{
		APortalCharacter* character = Cast<APortalCharacter>(PC->GetCharacter());
		CHECK_DESTROY(LogPortal, !pawn && !character, "Player portal pawn or character could not be found in the portal class %s.", *GetName());
		portalPawn = pawn;
	...
	lastPawnLoc = portalPawn ? portalPawn->camera->GetComponentLocation() : portalCharacter->camera->GetComponentLocation();
...
void APortal::UpdatePortalView()
{
	...
	portalCapture->PostProcessSettings = portalPawn ? portalPawn->camera->PostProcessSettings : portalCharacter->camera->PostProcessSettings;
...
void APortal::UpdatePawnTracking()
{
	...
	FVector currLocation = portalPawn ? portalPawn->camera->GetComponentLocation() : portalCharacter->camera->GetComponentLocation();
	...
	{
		// Teleport the actor.
		if (portalPawn) TeleportObject(portalPawn);
		else TeleportObject(portalCharacter);
	}
...
void APortal::UpdateTrackedActors()
{
	...
	if (actorsBeingTracked > 0)
	{
		...
		for (TMap<AActor*, FTrackedActor>::TIterator trackedActor = trackedActors.CreateIterator(); trackedActor; ++trackedActor)
		{
			...
			else if (APortalCharacter* isPlayerCharacter = Cast<APortalCharacter>(trackedActor->Key)) continue;
...
void APortal::TeleportObject(AActor* actor)
{
	...
	if (APortalCharacter* isCharacter = Cast<APortalCharacter>(actor))
	{
		isCharacter->LaunchCharacter(newLinearVelocity, true, true);
	}
	else
	{
	...
	else if (APortalCharacter* isCharacter = Cast<APortalCharacter>(actor))
	{
		isCharacter->PortalTeleport(pTargetPortal);
		portalCharacter->ReleaseInteractable();
	}
	else
	{
		// If the actor is grabbed by the pawn update the offset after teleporting.
		if (UPrimitiveComponent* isGrabbing = portalPawn ? portalPawn->physicsHandle->GetGrabbedComponent() : portalCharacter->physicsHandle->GetGrabbedComponent())
		{
			if (isGrabbing == (UPrimitiveComponent*)actor->GetRootComponent())
			{
				portalPawn ? portalPawn->ReleaseInteractable() : portalCharacter->ReleaseInteractable();
			}
		}
	}
	...
	pTargetPortal->lastPawnLoc = portalPawn ? portalPawn->camera->GetComponentLocation() : portalCharacter->camera->GetComponentLocation();
...
void APortal::CopyActor(AActor* actorToCopy)
{
	...
	else if (APortalCharacter* isCharacter = Cast<APortalCharacter>(newActor))
	{
		isCharacter->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_PortalBox, ECR_Ignore);
		isCharacter->GetCapsuleComponent()->SetCollisionResponseToChannel(ECC_Pawn, ECR_Ignore);
		isCharacter->GetCapsuleComponent()->SetSimulatePhysics(false);
		isCharacter->PrimaryActorTick.SetTickFunctionEnable(false);
	}
